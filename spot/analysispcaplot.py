...
def pca_plot(playlist):
    ...
    names = pl_frame['name'].to_frame()#.merge(coords)
    print names
    for i, txt in enumerate(names):
            ax.annotate(txt, (transformed[0,:][i], transformed[1,:][i]))
    #print names.merge(transformed)
    print pd.concat([names, coords], axis=1, join_axes=[names.index])
    #plt.show()



if __name__ == '__main__':
    playlist = spotify.pl_data('Smooth', 'rino21111')['songs']
    #print playlist
    pca_plot(playlist)
